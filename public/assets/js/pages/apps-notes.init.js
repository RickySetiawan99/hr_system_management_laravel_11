...
getJSON("notes-list.json", function (err, data) {
  if (err !== null) {
    ...
    const elements = document.querySelectorAll(
      '[data-bs-toggle="dropdown"].category-dropdown'
    );
...
function loadNotesData(datas, page) {
  ...
  if (datas.length > 0) {
    for (
      ...
      var activeNotes =
        datas[i].active == true
          ? "group/item toggle-button active"
          : "group/item toggle-button";
      document.querySelector("#notes-list").innerHTML +=
        `<div class="card product-item ` +
        calassCategory(datas[i].category) +
        `">\
                        ...
                        <div>
                            <div class="relative ltr:float-right rtl:float-left dropdown">
                                ...
                                <ul class="absolute z-50 hidden py-2 mt-1 text-left list-none bg-white rounded-md shadow-md dropdown-menu min-w-[10rem] dark:bg-zink-600" aria-labelledby="categoryNotes1">
                                    ...
                                    <li>
                                        <a data-modal-target="addNotesModal"  data-edit-id="` +
        datas[i].id +
        `" class="edit-item-btn block px-4 py-1.5 text-base transition-all duration-200 ease-linear text-slate-600 dropdown-item hover:bg-slate-100 hover:text-slate-500 focus:bg-slate-100 focus:text-slate-500 dark:text-zink-100 dark:hover:bg-zink-500 dark:hover:text-zink-200 dark:focus:bg-zink-500 dark:focus:text-zink-200" href="#!"><i data-lucide="file-edit" class="inline-block w-3 h-3 ltr:mr-1 rtl:ml-1"></i> <span class="align-middle">Edit</span></a>
                                    ...
                                    <li>
                                        <a data-modal-target="deleteModal" data-remove-id="` +
        datas[i].id +
        ...
        ` category-dropdown" id="notesAction1" data-bs-toggle="dropdown"  type="button" aria-expanded="false"></button>
...
function isCategory(val) {
  switch (val) {
    case "personal":
      return "w-4 h-4 mt-1 border border-dashed rounded-full dropdown-toggle shrink-0 bg-sky-100 border-sky-500 dark:bg-sky-500/20";
    case "business":
      return "w-4 h-4 mt-1 border border-dashed rounded-full dropdown-toggle shrink-0 bg-orange-100 border-orange-500 dark:bg-orange-500/20";
    case "social":
      return "w-4 h-4 mt-1 border border-dashed rounded-full dropdown-toggle shrink-0 bg-purple-100 border-purple-500 dark:bg-purple-500/20";
    case "home":
      return "w-4 h-4 mt-1 border border-dashed rounded-full dropdown-toggle shrink-0 bg-green-100 border-green-500 dark:bg-green-500/20";
  }
}
...
Array.prototype.slice.call(forms).forEach(function (form) {
  form.addEventListener("submit", function (event) {
    ...
    if (action == "edit") {
      ...
      document.querySelector("#notes-list").innerHTML +=
        ` <div class="card product-item ` +
        calassCategory(categoryInput.value) +
        `">\
                <div class="flex flex-col h-full card-body">
                    <div>
                        <div class="relative ltr:float-right rtl:float-left dropdown">
        ...
        `</p>
                    ...
                    <div class="flex items-center justify-between gap-3 pt-4 mt-auto">
                        ...
                        <p class="text-slate-500 shrink-0 dark:text-zink-200">` +
        dateValue +
        `</p>
                    </div>
                </div>
...
function editItem() {
  var getEditid = 0;
  var dateValue = new Date().toUTCString().slice(5, 16);
  Array.from(document.querySelectorAll(".edit-item-btn")).forEach(function (
    elem
  ) {
...
function removeItem() {
  var getid = 0;
  Array.from(document.querySelectorAll(".remove-item-btn")).forEach(function (
    item
  ) {
    item.addEventListener("click", function (event) {
      document
        .getElementById("remove-notes")
