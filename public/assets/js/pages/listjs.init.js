  ...
  var customerList = new List("customerList", options).on(
    ...
    function (list) {
      ...
      const totalRecordsElement =
        customerList.listContainer.querySelector(".total-records");
      const showingElement =
        customerList.listContainer.querySelector(".showing");

      if (totalEmploysElement)
        totalEmploysElement.innerHTML = customerList.items.length;
      if (totalRecordsElement)
        totalRecordsElement.innerHTML = customerList.items.length;

      if (showingElement)
        showingElement.innerHTML = customerList.visibleItems.length;
      document
...
xhttp.onload = function () {
  ...
  Array.from(json_records).forEach((raw) => {
    customerList.add({
      id:
        '<a href="javascript:void(0);" class="fw-medium link-primary id">#VZ' +
        raw.id +
        "</a>",
  ...
  if (ids_array.length > 0) {
    if (confirm("Are you sure you want to delete this?")) {
      ids_array.forEach(function (id) {
        ...
        customerList.remove(
          "id",
          `<a href="javascript:void(0);" class="fw-medium link-primary id">${id}</a>`
        );
      });
