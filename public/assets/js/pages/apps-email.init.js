...
function loadMailData(datas) {
  ...
  Array.from(datas).forEach(function (mailData, index) {
    ...
    document.querySelector("#mail-list").innerHTML +=
      `
        <tr class="relative before:absolute ltr:before:left-0 rtl:before:right-0 before:border [&.checked]:before:border-custom-500 before:inset-y-0 before:border-transparent ` +
      checkReaded +
      ` group/mail mail-card">
      ...
      `" class="itemCheckbox w-4 h-4 cursor-pointer bg-white border border-slate-200 checked:bg-none dark:bg-zink-700 dark:border-zink-500 rounded-sm appearance-none arrow-none relative after:absolute after:content-['\\eb7b'] after:top-0 after:left-0 after:font-remix after:leading-none after:opacity-0 checked:after:opacity-100 after:text-custom-500 checked:border-custom-500 dark:after:text-custom-500 dark:checked:border-custom-800" >
                <a href="#!" class="transition-all duration-200 ease-linear text-slate-500 hover:text-yellow-500 dark:text-zink-200 dark:hover:text-yellow-500 ` +
      checkStarred +
      `"><i data-lucide="star" class="w-4 h-4"></i></a>
        ...
        <td class="relative px-3.5 py-2.5 border-y border-transparent first:pl-0 last:pr-0 text-slate-500 group-[.unread]/mail:text-slate-800 dark:text-zink-200 dark:group-[.unread]/mail:text-zink-50  mailBox">
            <div class="grid items-center grid-cols-12 gap-3">
                <div class="col-span-4 lg:col-span-2">
                    <a href="#!" class="block truncate before:inset-0 before:absolute">` +
      mailData.name +
      `</a>
                </div>
      ...
      `</p>
        ...
        <td class="px-3.5 py-2.5 border-y border-transparent first:pl-0 last:pr-0 text-end w-20 text-slate-500 group-[.unread]/mail:text-slate-800 dark:text-zink-200 dark:group-[.unread]/mail:text-zink-50">` +
      mailData.time +
      `</td>
    </tr>`;
    lucide.createIcons();
...
function emailDetailShow() {
  // remove unread class
  Array.from(document.querySelectorAll("#mail-list tr td")).forEach(function (
    item
  ) {
    item.addEventListener("click", function (event) {
      Array.from(document.querySelectorAll("#mail-list tr.unread")).forEach(
        function (element) {
  ...
  Array.from(document.querySelectorAll("#closeChatRightSidebar")).forEach(
    function (item) {
...
function checkBoxAll() {
  document.querySelector("#checkboxAll").addEventListener("change", (e) => {
    Array.from(document.querySelectorAll(".itemCheckbox")).forEach(function (
      element
    ) {
  ...
  Array.from(document.querySelectorAll("#mail-list input")).forEach(function (
    element
  ) {
  ...
  function filterItems(arr, query) {
    return arr.filter(function (el) {
      return (
        ...
        el.subject.toLowerCase().indexOf(query.toLowerCase()) !== -1
      );
    });
  }
  ...
  removeItem.addEventListener("show.bs.modal", function (event) {
    document
      ...
      .addEventListener("click", function () {
        Array.from(document.querySelectorAll("#mail-list tr")).forEach(
          function (element) {
            var filtered = "";
