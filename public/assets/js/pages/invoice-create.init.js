...
function inputSpinComponents() {
  ...
  function updateQuantity(quantityInput) {
    ...
    Array.from(productRow.getElementsByClassName("item-line-price")).forEach(
      function (e) {
  ...
  function recalculateCart() {
    ...
    Array.from(elm).forEach(function (element) {
      Array.from(element.getElementsByClassName("item")).forEach(function (
        item
      ) {
        Array.from(item.getElementsByClassName("item-line-price")).forEach(
          function (e) {
    ...
    // set value in inputs
    const cartSubtitle = itemList.parentElement.querySelector(
      "#totalAmount .cart-subtotal"
    );
    if (cartSubtitle) {
      cartSubtitle.value = currencySign + subtotal.toFixed(2);
    }
    const cartTex = itemList.parentElement.querySelector(
      "#totalAmount .cart-tax"
    );
    if (cartTex) {
      cartTex.value = currencySign + tax.toFixed(2);
    }
    const cartShipping = itemList.parentElement.querySelector(
      "#totalAmount .cart-shipping"
    );
    if (cartShipping) {
      cartShipping.value = currencySign + shipping.toFixed(2);
    }
    const cartTotal = itemList.parentElement.querySelector(
      "#totalAmount .cart-total"
    );
    if (cartTotal) {
      cartTotal.value = currencySign + newTotal.toFixed(2);
    }
    const cartDiscountinvoce = itemList.parentElement.querySelector(
      ".item-list .cart-discount"
    );
    if (cartDiscountinvoce) {
      cartDiscountinvoce.value = "-" + currencySign + discount.toFixed(2);
    }
    const cartDiscount = itemList.parentElement.querySelector(
...
function new_link() {
  ...
  document
    ...
    .insertAdjacentHTML("beforeBegin", delLink);
  count++;
