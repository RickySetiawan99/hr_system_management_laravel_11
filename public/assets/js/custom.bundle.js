...
(function () {
  ...
  function inputSpinComponents() {
    ...
    function updateQuantity(quantityInput) {
      ...
      var price = parseFloat(
        productRow.querySelector(".product-price span")?.innerHTML || 0
      );
    ...
    function recalculateCart() {
      ...
      const cartSubtitle = elm.parentElement.querySelector(
      ...
      const cartShipping = elm.parentElement.querySelector(
        ".table-total .cart-shipping"
      );
      if (cartShipping) {
        cartShipping.innerHTML = currencySign + shipping.toFixed(2);
      }
      const cartTotal = elm.parentElement.querySelector(
        ".table-total .cart-total"
      );
      if (cartTotal) {
        cartTotal.innerHTML = currencySign + newTotal.toFixed(2);
      }
      const cartDiscount = elm.parentElement.querySelector(
        ".table-total .cart-discount"
      );
      if (cartDiscount) {
  ...
  function drawerSetting() {
    ...
    const allDrawerCloseButtons = document.querySelectorAll(
      "[data-drawer-close]"
    );
    const allModalButtons = document.querySelectorAll("[data-modal-target]");
    const allModalCloseButtons =
    ...
    if (document.getElementById("backDropDiv")) {
      ...
      backDropOverlay.className =
        "fixed inset-0 bg-slate-900/40 dark:bg-zink-800/70 z-[1049] backdrop-overlay hidden";
  ...
  function initTooltip() {
    ...
    allTooltipButtons.forEach((element) => {
      ...
      if (element.getAttribute("data-tooltip-arrow"))
        tippyOptions.arrow =
          element.getAttribute("data-tooltip-arrow") === "false"
            ? false
            : element.getAttribute("data-tooltip-arrow") === "true"
            ? true
            : element.getAttribute("data-tooltip-arrow");
      ...
      if (element.getAttribute("data-tooltip-follow-cursor"))
        tippyOptions.followCursor = element.getAttribute(
  ...
  function tabsComponents() {
    ...
    document.querySelectorAll(".nav-tabs").forEach(function (tabContainer) {
      ...
      tabContainer
        .querySelectorAll("[data-tab-toggle]")
        .forEach(function (tabItem) {
          ...
          tabItem.addEventListener("click", function () {
            ...
            var tabContents = document
              ...
              ?.parentElement.querySelectorAll(".tab-pane");
            if (tabContents) {
  ...
  function collapseComponent() {
    ...
    collapsibleSections.forEach((collapsible) => {
      const collapsibleHeader = collapsible.querySelector(
        ".collapsible-header"
      );
      const collapsibleContent = collapsible.querySelector(
        ".collapsible-content"
      );
    ...
    item.querySelectorAll(".dropdown-toggle").forEach(function (subitem) {
      subitem.addEventListener("click", function (event) {
        ...
        if (subitem.classList.contains("show") != true) {
          ...
          if (
            item.querySelector(".dropdown-menu").classList.contains("block")
          ) {
