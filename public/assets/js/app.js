...
function handleDropdownMenu() {
  ...
  dropdownToggleButtons.forEach((button, index) => {
      ...
      setTimeout(() => {
        ...
        if (
          document.documentElement.getAttribute("data-layout") ==
            "horizontal" &&
          subMenus.closest(".extra-menu") &&
          !subMenus.classList.contains("extra-menu")
        ) {
          if (dropdownMenu && subMenus) {
            ...
            if (isDropdownOffScreen) {
              if (
                subMenus.classList.contains(
                  "group-data-[layout=horizontal]:left-full"
                )
              ) {
                ...
                subMenus.classList.add(
                  "group-data-[layout=horizontal]:right-full"
                );
              } else {
                subMenus.classList.add(
                  "group-data-[layout=horizontal]:left-full"
    ...
    const subMenus = dropdownMenu.nextElementSibling
      ...
      : dropdownMenu.parentElement.nextElementSibling;
    if (
      subMenus.classList.contains("group-data-[layout=horizontal]:left-full") ||
      subMenus.classList.contains("group-data-[layout=horizontal]:right-full")
    ) {
      if (dropdownMenu && subMenus) {
        // get the position and dimensions of the dropdown menu
        var dropdownOffset = subMenus.getBoundingClientRect();
...
function removeActiveMenu(content) {
  document
    .querySelector("#scrollbar")
    ?.querySelectorAll(".dropdown-button.show")
    .forEach((aTag) => {
...
function updateParentActive(button) {
  if (button?.closest(".dropdown-content")) {
    ...
    updateParentActive(
      button.closest(".dropdown-content").previousElementSibling
    );
  }
}
...
function hideShowLayoutOptions(dataLayout) {
  if (dataLayout == "horizontal") {
    ...
    if (document.getElementById("customizerButton")) {
      ...
      document.documentElement.setAttribute(
        "data-navbar",
        sessionStorage.getItem("data-navbar")
      );
    }
  }
}
...
function setDefaultAttribute() {
  if (!sessionStorage.getItem("defaultAttribute")) {
    ...
    if (sessionStorage.getItem("data-layout") === "horizontal") {
      attributesToRetrieve = attributesToRetrieve.filter(
        (attribute) =>
          attribute !== "data-sidebar" && attribute !== "data-sidebar-size"
      );
    }
...
function layoutSetting() {
  ...
  const dataLayoutSkin = document.querySelectorAll(
    'input[name="dataLayoutSkin"]'
  );
...
function initFilters() {
  ...
  Array.from(filterMain).forEach(function (event) {
    ...
    const productItemsMain = document.getElementById(
      event?.getAttribute("data-filter-target")
    );
